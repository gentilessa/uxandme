...
export const caseStudyMeta = [
  ...
  { label: "Scope", value: "Seequent cloud design system" },
  { label: "Platforms", value: "Cloud web products and shared UI kit" },
  { label: "Products Supported", value: "Central, Web Viz, MySeequent" },
  { label: "Contribution Focus", value: "Foundations, components, layouts, implementation patterns" },
];

<div class="case-study-meta not-content" role="list">
  {caseStudyMeta.map(item => (
    <div class="case-study-meta__item" role="listitem">
      <span class="case-study-meta__label">{item.label}</span>
      <strong class="case-study-meta__value">{item.value}</strong>
    </div>
  ))}
</div>
